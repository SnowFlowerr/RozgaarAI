...
export default function Signup() {
    ...
    async function handleSubmit(e) {
        ...
        try {
            const userData = await axios.post("http://localhost:8000/api/auth/signup", { name: user.name, email: user.email, password: user.password, img: icon?.secure_url }, { withCredentials: true });
    ...
    const GoogleLogin = async (data) => {
        try{
            ...
            const response = await axios.post(`http://localhost:8000/api/auth/googlelogin`,
                {name:data.displayName, email:data.email, img:data.photoURL},
